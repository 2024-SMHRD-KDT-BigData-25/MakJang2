...
<body>
    ...
    <%
		...
		int totalCount;
		if (isSearch) {
		    totalCount = dao.getSearchFrBoardCount(searchType, searchKeyword); // 검색어에 맞는 게시글 수 조회
		} else {
		    totalCount = dao.getfrBoardCount(); // 전체 게시글 수 조회
		}
		...
		List<MyBoard> list;
		if (isSearch) {
		    list = dao.searchFrBoardListPaging(searchType, searchKeyword, startRow, endRow); // 검색된 게시글 가져오기
		} else {
		    list = dao.getfrBoardListPaging(startRow, endRow); // 전체 게시글 가져오기
		}
