...
<body>
    ...
    <%
		...
		int totalCount;
		if (isSearch) {
		    totalCount = dao.getSearchCrBoardCount(searchType, searchKeyword); // 검색어에 맞는 게시글 수 조회
		} else {
		    totalCount = dao.getcrBoardCount(); // 전체 게시글 수 조회
		}	
		...
		List<MyBoard> list;
		if (isSearch) {
		    list = dao.searchCrBoardListPaging(searchType, searchKeyword, startRow, endRow); // 검색된 게시글 가져오기
		} else {
		    list = dao.getcrBoardListPaging(startRow, endRow); // 전체 게시글 가져오기
		}
