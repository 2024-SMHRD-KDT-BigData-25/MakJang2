...
@WebServlet("/Board/create_BBS/viewBoard.do")
public class CrViewBoardServlet extends HttpServlet {
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {

        try {
            int boardId = Integer.parseInt(request.getParameter("No"));
            ...
            MyBoard board = dao.getcrBoard(boardId);

            if (board == null) {  // 게시글이 존재하지 않는 경우
                // 세션에 에러 메시지 저장
                request.getSession().setAttribute("errorMessage", "존재하지 않는 게시글입니다.");
                // 리스트 페이지로 리다이렉트
                response.sendRedirect("crList.jsp");  // 절대 경로로 수정
            } else {
                request.setAttribute("board", board);
                request.getRequestDispatcher("meetingview.jsp?No=" + boardId + "&pageNum=" + pageNum)
